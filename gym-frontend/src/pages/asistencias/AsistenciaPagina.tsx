...
// Interfaz alineada con las entidades del backend
interface AsistenciaPaginaProps {
    onLoginClick?: () => void;
    onRegistrarAsistencia?: (nombreCompleto: string) => void;
}

export default function AsistenciaPagina({ onLoginClick, onRegistrarAsistencia }: AsistenciaPaginaProps = {}) {
    // Variables de estado refactorizadas para coincidir con el backend (Entidad Cliente)
    const [nombreCompleto, setNombreCompleto] = useState('');
    const [asistenciaRegistrada, setAsistenciaRegistrada] = useState(false);
    const [nombreClienteConfirmado, setNombreClienteConfirmado] = useState(''); // Estado auxiliar para mostrar el nombre tras el registro
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Simular registro de asistencia (Entidad Asistencia)
        setTimeout(() => {
            setNombreClienteConfirmado(nombreCompleto);
            setAsistenciaRegistrada(true);
            setProcesando(false);

            // Llamar al callback usando la variable del backend
            if (onRegistrarAsistencia) {
                onRegistrarAsistencia(nombreCompleto);
            }
                    ...
                    {!asistenciaRegistrada ? (
                        <>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        id="nombreCompleto" // Alineado con entidad Cliente
                                ...
                                <button
                                    type="submit"
                                    disabled={procesando || !nombreCompleto.trim()}
                        ...
                        <div className="text-center py-8">
                            ...
                            <p className="text-gray-600 mb-1">
                                Bienvenido, {nombreClienteConfirmado}
                            </p>
