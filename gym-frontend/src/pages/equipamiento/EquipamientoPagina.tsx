...
// Interfaces alineadas a patrones del Backend
interface Equipamiento {
    equipoId: number;
    nombre: string;
    tipo: string;
    descripcion: string;
    imagenUrl?: string; // Propiedad opcional para la vista
}

interface EquipoAccesorio {
    accesorioId: number;
    nombre: string;
    cantidad: string; // O number, dependiendo de tu DB
    notas: string;
}

interface Mantenimiento {
    mantenimientoId: number;
    equipoID: number; // Foreign Key (ID en mayúsculas para consistencia con backend)
    descripcion: string;
...
interface EquipamientoPaginaProps {
    ...
    setEquipamiento: Dispatch<SetStateAction<Equipamiento[]>>;
    accesorios: EquipoAccesorio[];
    setAccesorios: Dispatch<SetStateAction<EquipoAccesorio[]>>;
    mantenimientos: Mantenimiento[];
    setMantenimientos: Dispatch<SetStateAction<Mantenimiento[]>>;
}
...
export default function EquipamientoPagina({
    ...
    const [editingEquipo, setEditingEquipo] = useState<Equipamiento | null>(null);

    const [formDataEquipo, setFormDataEquipo] = useState<Omit<Equipamiento, 'equipoId'>>({
        nombre: '',
        tipo: '',
        imagenUrl: '',
        descripcion: ''
    ...
    const [editingAccesorio, setEditingAccesorio] = useState<EquipoAccesorio | null>(null);
    const [formDataAccesorio, setFormDataAccesorio] = useState<Omit<EquipoAccesorio, 'accesorioId'>>({
        nombre: '',
        cantidad: '',
        notas: ''
    });
    ...
    // Usamos equipoID (Foreign Key)
    const [formDataMantenimiento, setFormDataMantenimiento] = useState<Omit<Mantenimiento, 'mantenimientoId' | 'equipo'>>({
        equipoID: 0,
        descripcion: '',
    ...
    const handleEditarEquipo = (equipo: Equipamiento) => {
    ...
    const handleGuardarEquipo = (e: React.FormEvent) => {
        ...
        if (editingEquipo) {
            ...
            const nuevoEquipo: Equipamiento = {
                ...formDataEquipo,
                equipoId: Date.now()
            };
    ...
    const handleEditarAccesorio = (accesorio: EquipoAccesorio) => {
    ...
    const handleGuardarAccesorio = (e: React.FormEvent) => {
        ...
        if (editingAccesorio) {
            ...
            const nuevoAccesorio: EquipoAccesorio = {
                ...formDataAccesorio,
    ...
    const handleEditarMantenimiento = (mantenimiento: Mantenimiento) => {
                                                ...
                                                {mantenimientosFiltrados.length === 0 ? (
                                                    ...
                                                    mantenimientosFiltrados.map((mantenimiento) => (
                                                        <tr key={mantenimiento.mantenimientoId} className="hover:bg-gray-50 transition-colors group">
                                                            ...
                                                            <td className="px-6 py-4 whitespace-nowrap">
                                                                <div className="text-sm text-gray-600">
                                                                    {new Date(mantenimiento.fechaInicio).toLocaleDateString()}
                                                                </div>
                                                            </td>
                                                            <td className="px-6 py-4 whitespace-nowrap">
                                                                <div className="text-sm text-gray-600">
                                                                    {mantenimiento.fechaFin
                                                                        ? new Date(mantenimiento.fechaFin).toLocaleDateString()
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        type="date"
                                        required
                                        value={formDataMantenimiento.fechaInicio as string}
                                ...
                                <div>
                                    ...
                                    <input
                                        type="date"
                                        value={formDataMantenimiento.fechaFin as string || ''}
