...
// Interfaces basadas en las clases del Backend
interface Pago {
    pagoId: number;
    membresiaID: number; // Foreign Key con mayúsculas
    monto: number;
...
interface Mantenimiento {
    ...
    equipoID: number; // Foreign Key con mayúsculas
    descripcion: string;
    ...
    // Relaciones
    equipo?: Equipamiento;
    estado?: 'En Curso' | 'Completado'; // Propiedad derivada para la vista
}

interface Membresia {
    membresiaId: number;
    tipoMembresiaID: number;
    usuarioID: number; // Se llama usuarioID en el backend
    fechaInicio: Date | string;
    // Relaciones
...
export default function FinanzasPagina({
    ...
    // Usamos membresiaID consistente con el backend
    const [formDataPago, setFormDataPago] = useState<Omit<Pago, 'pagoId'>>({
    ...
    const pagosConDetalles = pagos.map(pago => {
        ...
        // Buscar cliente usando usuarioID de la membresía
    ...
    const egresosConDetalles = mantenimientos.map(mant => {
        // Join con equipo usando equipoID
    ...
    const obtenerUltimasTransacciones = () => {
        ...
        pagos.forEach(pago => {
            ...
            transacciones.push({
                tipo: 'ingreso',
                fecha: pago.fechaPago as string,
        ...
        mantenimientos.forEach(mant => {
            ...
            transacciones.push({
                tipo: 'egreso',
                fecha: mant.fechaInicio as string,
                        ...
                        <form onSubmit={handleGuardarPago} className="p-6 space-y-5">
                            ...
                            <div>
                                ...
                                <input
                                    type="date"
                                    required
                                    value={formDataPago.fechaPago as string}
